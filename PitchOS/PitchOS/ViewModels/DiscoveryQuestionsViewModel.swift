...
@MainActor
@Observable
final class DiscoveryQuestionsViewModel {
    var questionsText = ""
    ...
    func generate() async {
        ...
        do {
            let stream = aiService.streamDiscoveryQuestions(profile: profile, deal: deal)

            for try await chunk in stream {
                questionsText += chunk.text
                tokenCount += 1
                if tokenCount % 20 == 0 {
